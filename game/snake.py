...
class Food(object):
    ...
    def draw(self, surface):
        r = pg.Rect((self.position[0], self.position[1]), (GRIDSIZE, GRIDSIZE))
        pg.draw.rect(surface, self.color, r)
        pg.draw.rect(surface, (93, 216, 228), r, 1)
...
def drawGrid(surface):
    for y in range(0, int(GRID_HEIGHT)):
        for x in range(0, int(GRID_WIDTH)):
            if  (x + y) % 2 == 0:
                r = pg.Rect((x*GRIDSIZE, y*GRIDSIZE), (GRIDSIZE, GRIDSIZE))
                pg.draw.rect(surface, (93, 216, 228), r)
            else:
                rr = pg.Rect((x*GRIDSIZE, y*GRIDSIZE), (GRIDSIZE, GRIDSIZE))
...
def main():
    ...
    clock = pg.time.Clock()
    screen = pg.display.set_caption("Snake Game")
